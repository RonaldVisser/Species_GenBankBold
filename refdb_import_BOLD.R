# added marker option to function (https://github.com/fkeck/refdb)
...
#
     ...
     if  (is.data.frame(recs)) {
          ...
          if(verbose) cat("No sequence found (if marker was given, species had no data with marker).\n")
          return(NULL)
     }
     if (nrow(recs) == 1 && !"processid" %in% colnames(recs)) {
          if (grep("You have exceeded your allowed request quota.", names(recs)) == 1) {
               stop("\n You have exceeded your allowed request quota. \n If you wish to download large volume of data, please contact support@boldsystems.org for instruction on the process.")
          }
     }
     ...
     out <- refdb::refdb_set_fields_BOLD(out)
     out <- refdb::refdb_set_fields(out,
                             latitude = "lat",
                             longitude = "lon")

     if (ncbi_taxo) {
          out <- refdb::refdb_set_ncbitax(out)
     }
